...
const sizes = [
  { id: 'small', label: 'Small', preview: 13 },
  { id: 'medium', label: 'Medium', preview: 15 },
...
const FontSizeSheet = ({ open, onOpenChange, value, onApply }: FontSizeSheetProps) => {
    ...
    <Drawer open={open} onOpenChange={(o) => { if (!o) setSelected(value); onOpenChange(o); }}>
      <DrawerContent className="max-h-[90vh]">
        <DrawerHeader className="flex items-center justify-between px-5 pb-2">
          <DrawerTitle className="text-[17px] font-bold" style={{ color: '#1A2332' }}>Font Size</DrawerTitle>
          <DrawerClose asChild>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1">
                  <div className="text-[14px] font-medium" style={{ color: '#1A2332' }}>{s.label}</div>
                  <div style={{ fontSize: s.preview, color: '#475569', marginTop: 2 }}>Aa</div>
                </div>
              </button>
            );
          })}
