...
const providers = [
  { id: 'anthropic', label: 'Anthropic (Claude)', subtitle: 'Recommended' },
  { id: 'openai', label: 'OpenAI (GPT)' },
  { id: 'custom', label: 'Other / Custom', subtitle: 'Enter custom endpoint' },
];

interface AIProviderSheetProps {
  open: boolean;
...
const AIProviderSheet = ({ open, onOpenChange, value, onApply }: AIProviderSheetProps) => {
    ...
    <Drawer open={open} onOpenChange={(o) => { if (!o) setSelected(value); onOpenChange(o); }}>
      <DrawerContent className="max-h-[90vh]">
        <DrawerHeader className="flex items-center justify-between px-5 pb-2">
          <DrawerTitle className="text-[17px] font-bold" style={{ color: '#1A2332' }}>AI Provider</DrawerTitle>
          <DrawerClose asChild>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1">
                  <div className="text-[14px] font-bold" style={{ color: '#1A2332' }}>{p.label}</div>
                  {p.subtitle && <div className="text-[12px]" style={{ color: '#6B7C93' }}>{p.subtitle}</div>}
                </div>
              </button>
            );
          })}
...
export default AIProviderSheet;
